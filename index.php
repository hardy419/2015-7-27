...
<div id="maincontent">
	...
	?>
		<div class="element pict" style='background:#FFFFFF;'>
			<?php if($rel_arr_banner['doc_type']!='OA' && $rel_arr_banner['doc_type']!='LA'){ ?>
				<a target='<?php echo $rel_arr_banner['link_open_window'] == 'Y' ? '_blank' : '_self';?>' href="./news_detail.php?id=<?php echo $rel_arr_banner['id'];?>&tp=<?php echo $rel_arr_banner[0];?>">
					<?php echo '<img src="./userUpload/banner/'.$rel_arr_banner['banner_photo'].'">'?>
				</a>
			<?php }else{ ?>
				<?php if($rel_arr_banner['doc_type'] == 'OA'){	?>
						<a target='<?php echo $rel_arr_banner['link_open_window'] == 'Y' ? '_blank' : '_self'; ?>' href="./process_achievement_details.php?id=<?php echo $rel_arr_banner['id']; ?>">
							<?php echo '<img src="./userUpload/banner/'.$rel_arr_banner['banner_photo'].'">'; ?>
						</a>
				<?php }else{ ?>
						<a target='<?php echo $rel_arr_banner['link_open_window'] == 'Y' ? '_blank' : '_self'; ?>' href="./process_study_details.php?id=<?php echo $rel_arr_banner['id']; ?>">
							<?php echo '<img src="./userUpload/banner/'.$rel_arr_banner['banner_photo'].'">'; ?>
						</a>
				<?php } ?>
			<?php } ?>
...
<table width="1000" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-bottom:20px; margin-top:20px;" class="index_news">
  <tr>
    <td width="510" valign="top">
		...
		<?php include('./model/news_list_selection_homepage.php'); ?>
		<?php $i = 0;?>
      ...
      <table width="510" border="0" cellspacing="0" cellpadding="0" style="margin-bottom:13px;">
        <tr>
		  ...
		  ?>
		  <?php 
		  if(!empty($rel_arr_news['file'])){
			echo '<a target="_blank" href="./userUpload/attachment/'.$rel_arr_news['file'].'">'.get_sub_string($rel_arr_news['title_cn'],50).'</a>';
		  }else if(!empty($rel_arr_news['link_url'])){
		  ?>
		  ...
		  <?php
				...
				if($rel_arr_news[0]==3){
					if($rel_arr_news['web_type']=='9'){
						echo '<a target="_self" href="./process_study.php'.'">'.get_sub_string($rel_arr_news['title_cn'],50).'</a>';
					}else if($res_doc_type['docoment_type']=='PC'){
						echo '<a target="_self" href="./community_homeschool_detail.php?id='.$rel_arr_news['id'].'">'.get_sub_string($rel_arr_news['title_cn'],50).'</a>';
					}else if($res_doc_type['docoment_type']=='OA'){
						echo '<a target="_self" href="./process_achievement_details.php?id='.$rel_arr_news['id'].'">'.get_sub_string($rel_arr_news['title_cn'],50).'</a>';
					}else if($res_doc_type['docoment_type']=='LA'){
						echo '<a target="_self" href="./process_study_details.php?id='.$rel_arr_news['id'].'">'.get_sub_string($rel_arr_news['title_cn'],50).'</a>';
					}
				}else{ 
					echo'<a target="_self" href="./news_detail.php?id='.$rel_arr_news['id'].'&tp='.$rel_arr_news[0].'">'.get_sub_string($rel_arr_news['title_cn'],50).'</a>';
				}
		  }?></td>
        </tr>
        <tr>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
          <td height="18" align="left"><a target='_self' href="<?php if(!empty($rel_arr_news['file_name'])){ echo './userUpload/attachment/'.$rel_arr_news['file_name'];
		  }else{
		  		if($rel_arr_news[0]==4){
					echo './photos_details.php?id='.$rel_arr_news['id'];
						...
						}else if($res_doc_type['docoment_type']=='SN'){
							echo './userUpload/attachment/'.$rel_arr_news['file'];
						}else{
							echo './userUpload/attachment/'.$rel_arr_news['file'];
						}
					}else if($rel_arr_news[0]==1){
						echo './process_works_details.php?id='.$rel_arr_news['id'];
					}else{ 
