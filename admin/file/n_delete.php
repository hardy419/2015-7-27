...
if( $search_Obj=mysql_fetch_object($search_Result) )
{
	...
	if( $action == 3 )
	{
		...
		$del_SQL = " UPDATE  tbl_file  SET  file_photo=''  WHERE  file_id=".$file_id;
	...
	else if( $action == 2 )
	{
		...
		$del_SQL = " UPDATE  tbl_file  SET  file_file_name=''  WHERE  file_id=".$file_id;
		mysql_query( $del_SQL, $link_id );
		$msg = "RWɮק";
	...
	else
	{
		...
		$del_SQL = " DELETE FROM  tbl_file   WHERE  file_id=".$file_id;
		mysql_query( $del_SQL, $link_id );
