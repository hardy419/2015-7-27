...
if($a_file==""){
$update_sql = "UPDATE `tbl_other` SET
`a_title`='$u_name',`a_type`='$a_type', `a_content`='$a_content',`a_date`='$date', `order`='$order' WHERE `a_id`=".$a_id;
}else{
$update_sql = "UPDATE `tbl_other` SET
`a_title`='$u_name',`a_type`='$a_type', `a_content`='$a_content', `order`='$order',`down_file`='$a_file'  WHERE `a_id`=".$a_id;
}
