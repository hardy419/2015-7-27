...
$img_sql = "SELECT ta.type_id FROM `tbl_lastest_gallery` AS tg , 
`tbl_lastest` AS ta  WHERE tg.act_id=ta.id AND tg.file_name='".$file_name."'
...
if( $img_obj = mysql_fetch_object($img_result) )
{
	...
	$update_sql = "UPDATE `tbl_lastest_gallery` SET
	`remark`='$remark',
	`g_order`=$g_order 
