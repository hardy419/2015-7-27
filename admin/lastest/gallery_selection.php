...
// Get Photo
$get_rs = mysql_query("select `id`,`name` from tbl_lastest where id=".$id, $link_id);
$get_rows=mysql_fetch_row($get_rs);
$get_sql = "SELECT ta.name, ta.type_id, tg.* FROM `tbl_lastest_gallery` AS tg , 

`tbl_lastest` AS ta  WHERE tg.act_id=ta.id AND tg.act_id=".$id."
...
$get_result3 = mysql_query(" SELECT * FROM  tbl_lastest  AS  ta LEFT JOIN tbl_art AS tt ON(ta.type_id=tt.type_id) 

LEFT JOIN  `tbl_lastest_gallery` AS tg   ON( ta.id=tg.act_id  )
