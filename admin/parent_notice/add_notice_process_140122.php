<?
	...
	if (isset($_POST[Submit])){
		...
		if($_POST['is_news'] == 'Y' && $banner_file_status == '100'){
			...
			VALUES ('$date', '$date2', '$_POST[no]', '$_POST[title_cn]', '$_POST[title_en]', '$description' , '', '$_POST[type]', '$_POST[child_type]', '$_POST[school_year]', '$_POST[date_month]' , '$_SESSION[name]', now(), '$_POST[link_text]', '$_POST[link_url]', '$_POST[new_window]', '$category', '$_POST[is_news]', '$_POST[web_type]', '$_POST[target]' , '$_POST[order]' , '$_POST[is_banner]' , '$_POST[banner_order]' , '$file_name' , '$_POST[acc_type]');";
		}else{
			$add_sql = "INSERT INTO `tbl_notice` (`date` , `exp_date` , `serialno` , `title_cn` , `title_en` , `description` , `docoment_name` , `docoment_type` , `doc_child_type` , `school_year` , `month`, `modified_by`, `modified_date` , `link_text` , `link_url` , `link_open_window`, `category`, `is_news` , `web_type` , `target` , `access_type`) 
			VALUES ('$date', '$date2', '$_POST[no]', '$_POST[title_cn]', '$_POST[title_en]', '$description' , '', '$_POST[type]', '$_POST[child_type]', '$_POST[school_year]', '$_POST[date_month]' , '$_SESSION[name]', now(), '$_POST[link_text]', '$_POST[link_url]', '$_POST[new_window]', '$category', '$_POST[is_news]', '$_POST[web_type]', '$_POST[target]' , '$_POST[acc_type]');";
		}
	...
	if (isset($_FILES["cover_Photo"]["tmp_name"])&&$_FILES["cover_Photo"]["tmp_name"]!=""){	
		$output_path_cover="../../userUpload/cover_Photo/";
	    	if (!copy($_FILES["cover_Photo"]["tmp_name"],$output_path_cover.$new_file_name_cover)){
			echo "Fail to copy doc file - ". $_FILES["cover_Photo"]["tmp_name"];
			exit();			
		}else{
	...
	//echo $add_sql;
	header("Location:c_parent.php?msg=$msg&school_year=$_POST[school_year]&month=$_POST[date_month]");
    }
?>