...
// calendarid post_id poster date title content posttime 
	...
	if($_POST['is_news'] == 'Y' && $banner_file_status == '100'){
		$add_sql = "update `tbl_notice` set `date` ='$date' , `exp_date` = '$date2' , `serialno` ='$_POST[no]', `title_cn` ='$_POST[title_cn]', `title_en` ='$_POST[title_en]' , `description` = '$_POST[description]' , `docoment_type` = '$_POST[type]' , `doc_child_type` = '$_POST[child_type]', `month` ='$_POST[date_month]', `school_year` = '$_POST[school_year]' , `modified_by` = '$_SESSION[name]' , `modified_date` = now(), `link_text` = '$_POST[link_text]', `link_url` = '$_POST[link_url]', `link_open_window` = '$_POST[new_window]', `category` = '$category', is_news ='$_POST[is_news]', web_type ='$_POST[web_type]' , target ='$_POST[target]' , `news_order` = '$_POST[news_order]' , is_banner = '$_POST[is_banner]' , banner_order = '$_POST[banner_order]' , banner_photo = '$file_name' , `access_type` = '$_POST[acc_type]' where `noticeid`  = '$_GET[id]'";
	}else{
		$add_sql = "update `tbl_notice` set `date` ='$date' , `exp_date` = '$date2' , `serialno` ='$_POST[no]', `title_cn` ='$_POST[title_cn]', `title_en` ='$_POST[title_en]' , `description` = '$_POST[description]' , `docoment_type` = '$_POST[type]' , `doc_child_type` = '$_POST[child_type]', `month` ='$_POST[date_month]', `school_year` = '$_POST[school_year]' , `modified_by` = '$_SESSION[name]' , `modified_date` = now(), `link_text` = '$_POST[link_text]', `link_url` = '$_POST[link_url]', `link_open_window` = '$_POST[new_window]', `category` = '$category', is_news ='$_POST[is_news]', web_type ='$_POST[web_type]' , target ='$_POST[target]' , `access_type` = '$_POST[acc_type]' where `noticeid`  = '$_GET[id]'";
	}
	...
	if (isset($_FILES["cover_Photo"]["tmp_name"])&&$_FILES["cover_Photo"]["tmp_name"]!=""){
		...
		if ($get_cover_rows[file_name] != "")
	    	...
	    	if (!copy($_FILES["cover_Photo"]["tmp_name"],$oldfilename_cover.$new_file_name_cover)){
