...
// Get Photo
$get_rs = mysql_query("select `id`,`name` from tbl_art where id=".$id, $link_id);
$get_rows=mysql_fetch_row($get_rs);
$get_sql = "SELECT ta.name, ta.type_id, tg.* FROM `tbl_art_gallery` AS tg , 
...
$get_result3 = mysql_query(" SELECT * FROM `tbl_art_type` AS tt  

LEFT JOIN   tbl_art  AS  ta  ON(ta.type_id=tt.type_id) 

LEFT JOIN  `tbl_art_gallery` AS tg   ON( ta.id=tg.act_id  )
