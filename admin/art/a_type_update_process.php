...
{
	$sql = 'INSERT INTO `tbl_art_type` ( `type_name`, `type_order` )  VALUES ( "'. $type_name .'", '.$type_order.' )';
...
{
	$sql = " UPDATE `tbl_art_type` SET      type_name='". $type_name ."' ,   type_order=". $type_order ."   WHERE    type_id=". $type_id;
	mysql_query("set names utf8");
	if ( mysql_query( $sql, $link_id ) )
...
{
	$sql = " DELETE  FROM  tbl_art_type  WHERE   type_id = ". $type_id;
	mysql_query("set names utf8");
	if ( mysql_query( $sql, $link_id ) )
		$msg = " R\ ";
	else
