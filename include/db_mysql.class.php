...
class db_mysql {
    ...
    function db_mysql($host = '', $name = '', $user = '', $pass = '', $charset = 'utf8',	$pconnect=0, $debug = false) {
        ...
        if ($host != '') {
            ...
            if (is_resource($this->connect($host, $user, $pass,$pconnect))) {
    ...
    function connect($host, $user, $pass,$pconnect) {
    ...
    function my_query($sql='', $type = 'assoc') {

    	$this->select_db($this->name);

    	$res = $this->query($sql, $type);
    ...
    function my_num_rows($sql='',$type='assoc'){
        ...
        $res = mysql_num_rows($this->queryid);
        ...
        if (is_resource($this->queryid)) {
            ...
            if (mysql_num_rows($this->queryid) == 0) {
                ...
                $this->result = 0;
                      ...
                      preg_match("/limit\s+1$|limit\s+1\s+|limit\s*0\s*,\s*1$/i", $this->sql)) {
	...
	function table_fields($table){
		...
		$res = @mysql_query('select * from '.$table.' LIMIT 0, 1', $this->linkid);

		$i=0;

		while ($i<mysql_num_fields($res)) {

			$field_names[]=mysql_fetch_field($res);
